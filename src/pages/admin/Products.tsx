...
type ProductFormState = ProductRecord & {
  name: string;
  img: string;
  price: string | number;
  description: string;
...
const productFields: Array<keyof ProductFormState> = [
  'name',
  'img',
  'price',
  'description',
...
const buildProductFormData = (product: ProductFormState, imageFile: File | null) => {
            ...
            {products.map((item) => (
              <div key={item._id || item.id} className={adminProductsStyles.card}>
                ...
                <div className={adminProductsStyles.actionRow}>
                  <button
                    onClick={() => {
                      setSelectedProduct({ ...createEmptyProduct(), ...item });
                      setSelectedImageFile(null);
                      setOpen(true);
                    }}
