...
const extractHexColors = (styleObject: Record<string, unknown>): string[] => {
  const hexPattern = /#(?:[0-9a-fA-F]{3}){1,2}/g;

  return Object.values(styleObject).flatMap((value) => {
...
const getReadabilityLabel = (ratio: number) => {
...
const analyzeColor = (hex: string) => ({
  hex,
  luminance: Number(getLuminance(hex).toFixed(4)),
});

const analyzeStyleObject = (styleObject: Record<string, unknown>, baseBackground: string) => {
  return extractHexColors(styleObject).map((hex) => ({
    ...analyzeColor(hex),
